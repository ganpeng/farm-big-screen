...
export default {
  ...
  methods: {
    ...
    async mapInit() {
      try {
        ...
        this.districtCluster.on(
          "clusterMarkerClick",
          this.clusterMarkerClickHandler.bind(this)
        );
        this.districtCluster.on(
          "clusterMarkerMouseover",
          this.clusterMarkerMouseoverHandler.bind(this)
        );
        this.districtCluster.on(
          "clusterMarkerMouseout",
          this.clusterMarkerMouseoutHandler.bind(this)
        );
        this.pointSimplifier.on(
          "pointClick",
          this.pointSimplifierClickHandler.bind(this)
        );
        this.pointSimplifier.on(
          "pointMouseout",
      ...
      return new this.DistrictCluster({
        ...
        renderOptions: {
          ...
          getClusterMarker: (feature, dataItems, recycledMarker) => {
            ...
            // <img src="${this.markIcon}" class="mark-icon">
            var label = {
    ...
    async clusterMarkerMouseoverHandler(e, record) {
      try {
        const result2 = this.districtCluster.getAreaNodeProps(record.adcode);
        const center = result2.center;
        this.myInfoWindow.setContent(this.getMarkInfoWindowContent(record));
        this.myInfoWindow.open(this.map, center);
      } catch (err) {
        console.log(err);
      }
    },
    async clusterMarkerMouseoutHandler(e, record) {
      try {
      } catch (err) {
        console.log(err);
      }
    },
    async pointSimplifierClickHandler(e, record) {
      try {
        const center = record.data.dataItem.split(",");
        this.myInfoWindow.setContent(this.getPointSimplifierContent(record));
        this.myInfoWindow.open(this.map, center);
      } catch (err) {
        console.log(err);
      }
    },
    async pointSimplifierMouseoutHandler() {
      try {
      } catch (err) {
        console.log(err);
      }
    ...
    gotoFarmDetail() {
      this.$router.push({ name: "FarmDetail", query: { a: 1 } });
    },
    getMarkInfoWindowContent(data) {
      const name = data.feature.properties.name;
      return `<div class="mark-info-window-container">
                        <div class="title">${name}农场概况</div>
                        <ul class="mark-info-list">
                            <li class="mark-info-item">覆盖农场：212,737个</li>
                            <li class="mark-info-item">农场覆盖率：50%</li>
                            <li class="mark-info-item">覆盖耕地面积：1400万亩</li>
                            <li class="mark-info-item">耕地覆盖率：50%</li>
                        </ul>
                    </div>`;
    },
    // 地图层级缩放时候的事件
    ...
    setCity(val) {
      ...
      // 用map.setCity(xx)可以通过中文城市名、adcode、citycode等设置地图的中心点
